...
data class GetManager2ReviewStatusParams(
  val organisationId: Long?,
  val reviewCycleId: Long?
)

class GetManager2ReviewStatusParamSetter : ParamSetter<GetManager2ReviewStatusParams> {
  override fun map(ps: PreparedStatement, params: GetManager2ReviewStatusParams) {
    ps.setObject(1, params.organisationId)
    ps.setObject(2, params.reviewCycleId)
  }
}

data class GetManager2ReviewStatusResult(
  val inProgress: Long?,
  val completed: Long?
)

class GetManager2ReviewStatusRowMapper : RowMapper<GetManager2ReviewStatusResult> {
  override fun map(rs: ResultSet): GetManager2ReviewStatusResult = GetManager2ReviewStatusResult(
  inProgress = rs.getObject("in_progress") as kotlin.Long?,
    completed = rs.getObject("completed") as kotlin.Long?)
}

class GetManager2ReviewStatusQuery : Query<GetManager2ReviewStatusParams,
    GetManager2ReviewStatusResult> {
  ...
  override val mapper: RowMapper<GetManager2ReviewStatusResult> = GetManager2ReviewStatusRowMapper()

  override val paramSetter: ParamSetter<GetManager2ReviewStatusParams> =
      GetManager2ReviewStatusParamSetter()
}
